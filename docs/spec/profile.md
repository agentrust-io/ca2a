...
A requirement stated here that this implementation does not yet meet is marked inline. The profile states the requirement; it does not describe the implementation as conformant where it is not. See [LIMITATIONS.md](../../LIMITATIONS.md).
...
A cA2A implementation MUST NOT change A2A wire formats, routing, or endpoints. Removing every cA2A field from a message MUST leave a valid A2A task. cA2A data MUST travel in A2A extension fields as defined in [transport](transport.md).
...
The official-SDK bridge supplies this declaration from live `PeerNode` state for
an operator to merge into its Agent Card before signing. It does not fabricate a
complete card for the reference server, which has no operator identity,
interface, skill, or public URL to claim. Client-side inspection records the
declaration without yet imposing a missing-card refusal policy; see the
[transport binding](transport.md#declaring-the-extension-with-the-official-sdk).
...
An implementation claiming conformance MUST satisfy every MUST in this document. [conformance.md](conformance.md) maps these requirements onto the runnable checks in `tests/conformance/`, which is what a "cA2A-compatible" claim is measured against.

## Stability

This binding targets A2A v1.x extension points. Confirming that those extension points are stable enough to bind to is a precondition on the roadmap; see [ROADMAP.md](../../ROADMAP.md).
